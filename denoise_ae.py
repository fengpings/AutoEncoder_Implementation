...
from code0809.mnist import NoiseAEMNIST
...
if __name__ == '__main__':
    ...
    trainset = NoiseAEMNIST(root='../data/MNIST', train=True, download=True, transform=transform)
    trainloader = DataLoader(trainset, batch_size=_batch_size, shuffle=True, num_workers=2, prefetch_factor=_batch_size)
    testset = NoiseAEMNIST(root='../data/MNIST', train=False, download=True, transform=transform)
    ...
    # writer = SummaryWriter(log_dir='./output/denoise_ae')
    ...
    for epoch in range(total_epoch):
        ...
        img0, img1, label = testset[idx]  # 原始图像，噪声图像，标签
        img = copy.deepcopy(img1)
        img = img[None, ...]  # 噪声图像
        img0 = img0[None, ...]  # 原始清晰图像
        img1 = net(img1[None, ...])  # 恢复去噪后的图像
        z = torch.from_numpy(np.random.normal(0.0, 1.0, size=(1, 32))).float()
        img2 = net.generate(z)  # 基于随机数生成图像
        img = torch.cat([img, img0, img1, img2], dim=0)
        _dir = f'./output/denoise_ae/image'
        if not os.path.exists(_dir):
            os.makedirs(_dir)
        utils.save_image(img, f'{_dir}/{epoch}_{label}.png')

    torch.save(net, './output/denoise_ae/model.pth')
